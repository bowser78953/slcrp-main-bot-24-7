...
def _rainbow_color_value() -> int:
    # Sweep hue over time to approximate a rainbow embed color.
...
def _build_giveaway_embed(giveaway: dict) -> discord.Embed:
    ...
    embed.add_field(
        name="<:Winners:1525734637383450634> Winners",
        value=str(giveaway["winner_count"]),
        inline=True,
    )
    embed.add_field(
        name="<:Entrees:1525734579099533413> Entrees",
        value=str(entries_count),
        inline=True,
    )
    embed.add_field(
        name="<:Time:1525734537940701266> Time",
        value=time_display,
        inline=True,
    )
    return embed
...
async def _fetch_stock_lines_and_next_restock() -> tuple[list[str], list[str], str | None, int | None, str | None, list[str]]:
    ...
    for entry in SEED_CONFIG:
        ...
        if rarity and (
            best_rarity is None
            or RARITY_RANK.get(str(rarity), 0) > RARITY_RANK.get(str(best_rarity), 0)
        ):
...
async def _ensure_seed_shop_live_message_exists() -> None:
    ...
    # Post one live snapshot on startup, then the loop posts on each detected restock.
    lines, gear_lines, next_restock_text, next_restock_unix, best_rarity, role_mentions = await _fetch_stock_lines_and_next_restock()
...
async def on_ready():
    ...
    if not TREE_SYNCED:
        ...
        # Keep guild and global trees aligned to avoid stale command-id mismatches.
...
@app_commands.describe(
    ...
    amount_of_winners="How many winners",
)
async def giveaway_slash(
    ...
    description: str,
    time: str,
    amount_of_winners: app_commands.Range[int, 1, 100],
):
    try:
        duration_seconds = _parse_duration_to_seconds(time)
    except ValueError:
        await interaction.response.send_message(
            "Invalid time format. Use like 1d_1h_1m_1s.",
            ephemeral=True,
        )
        return
...
async def sreport(ctx: commands.Context, user: discord.Member, *, reason: str):
    ...
    if isinstance(scam_channel, discord.TextChannel):
        ...
        await ctx.send(
            f"I could not access <#{SCAM_REPORT_CHANNEL_ID}> to post the report message."
        )
        return
...
async def vouchlist(ctx: commands.Context, user: discord.Member):
    ...
    embed = discord.Embed(
        title=f"{user.display_name} Vouch and Scam Reports",
