...
TOKEN = os.getenv("TCF_BOT_TOKEN", "")
PREFIX = os.getenv("TCF_BOT_PREFIX", "!")
STATUS_TEXT = os.getenv("TCF_BOT_STATUS", "Running TCF Bot")
...
async def on_ready() -> None:
    ...
    print(f"TCF Bot is online as {bot.user} (ID: {bot.user.id})")
...
def main() -> None:
    if not TOKEN:
        raise RuntimeError("Missing TCF_BOT_TOKEN in environment variables or .env file")
    bot.run(TOKEN)
